/*
  ...
  This example is written for the SparkFun Thing Plus C (ESP32) but will work on any ESP32 board.
  writeConfigurationToStorage and readConfigurationFromStorage write to and read from a file called "OLconfig.csv"
  using LittleFS. Please see the previous example if you want to use microSD to store the configuration.
...
void setup()
{
  ...
  if (!mySensors.beginStorage((const char *)configurationFileName))
  {
  ...
  mySensors.theMenu.addMenuItem("Write the sensor configuration to LittleFS", writeConfig);
  mySensors.theMenu.addMenuItem("Read the sensor configuration from LittleFS", readConfig);
  mySensors.theMenu.addMenuItem("", SFE_QUAD_MENU_VARIABLE_TYPE_NONE);
