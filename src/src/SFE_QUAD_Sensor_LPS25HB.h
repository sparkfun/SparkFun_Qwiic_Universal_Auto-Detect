...
public:
  ...
  bool getSenseReading(uint8_t sense, char *reading)
  {
    ...
    {
    case 0:
      _sprintf._dtostrf((double)device->getPressure_hPa(), reading); // Get the pressure
      return (true);
      break;
    case 1:
      _sprintf._dtostrf((double)device->getTemperature_degC(), reading); // Get the temperature
      return (true);
      break;
    default:
      return (false);
