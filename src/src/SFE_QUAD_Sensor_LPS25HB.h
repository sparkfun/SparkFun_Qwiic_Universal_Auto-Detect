#ifndef SPARKFUN_QUAD_SENSOR_LPS25HB_H // <=== Update this with the new sensor type
#define SPARKFUN_QUAD_SENSOR_LPS25HB_H // <=== Update this with the new sensor type

#include "LPS25HB/SparkFun_LPS25HB_Arduino_Library.h" // <=== Update this with the new sensor library header file
...
#define CLASSNAME LPS25HB // <=== Update this with the new sensor type

#define CLASSTITLE SFE_QUAD_Sensor_LPS25HB // <=== Update this with the new sensor type
...
#define SENSOR_I2C_ADDRESSES const uint8_t sensorI2cAddresses[] = {0x5C, 0x5D} // <=== Update this with the I2C addresses for this sensor
...
public:
  ...
  // Return the name of the name of the specified sense
  ...
  bool getSenseReading(uint8_t sense, char *reading)
  {
    ...
    {
    case 0:
      OLS_sprintf.OLS_etoa((double)device->getPressure_hPa(), reading); // Get the pressure
      return (true);
      break;
    case 1:
      OLS_sprintf.OLS_etoa((double)device->getTemperature_degC(), reading); // Get the temperature
      return (true);
      break;
    default:
      return (false);
