#ifndef SPARKFUN_QUAD_SENSOR_HEADER_MS8607_H // <=== Update this with the new sensor type
#define SPARKFUN_QUAD_SENSOR_HEADER_MS8607_H // <=== Update this with the new sensor type

#include "MS8607/SparkFun_PHT_MS8607_Arduino_Library.h" // <=== Update this with the new sensor library header file
...
#define CLASSNAME MS8607 // <=== Update this with the new sensor type

#define CLASSTITLE SFE_QUAD_Sensor_MS8607 // <=== Update this with the new sensor type
...
#define SENSOR_I2C_ADDRESSES const uint8_t sensorI2cAddresses[] = {0x40} // <=== Update this with the I2C addresses for this sensor
...
public:
  ...
  bool getSenseReading(uint8_t sense, char *reading)
  {
    ...
    {
    case 0:
      _sprintf._dtostrf((double)device->getPressure(), reading); // Get the pressure
      return (true);
      break;
    case 1:
      _sprintf._dtostrf((double)device->getTemperature(), reading); // Get the temperature
      return (true);
      break;
    case 2:
      _sprintf._dtostrf((double)device->getHumidity(), reading); // Get the temperature
      return (true);
      break;
    default:
      return (false);
  ...
  const char *getSettingName(uint8_t setting)
  {
    ...
    {
    case 0:
      return ("Enable Heater");
      break;
    case 1:
      return ("Disable Heater");
      break;
    default:
      return (NULL);
      break;
  ...
  bool getConfigurationItem(uint8_t configItem, SFE_QUAD_Sensor_Every_Type_t *value)
  {
    ...
    {
    case 0:
      enum MS8607_heater_status status;
      device->get_heater_status(&status);
      value->BOOL = status == MS8607_heater_on;
      break;
    default:
      return (false);
      break;
