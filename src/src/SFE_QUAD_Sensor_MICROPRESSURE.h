#ifndef SPARKFUN_QUAD_SENSOR_HEADER_MICROPRESSURE_H // <=== Update this with the new sensor type
#define SPARKFUN_QUAD_SENSOR_HEADER_MICROPRESSURE_H // <=== Update this with the new sensor type

#include "MICROPRESSURE/SparkFun_MicroPressure.h" // <=== Update this with the new sensor library header file
...
#define CLASSNAME SparkFun_MicroPressure // <=== Update this with the new sensor type

#define CLASSTITLE SFE_QUAD_Sensor_MICROPRESSURE // <=== Update this with the new sensor type

#define SENSE_COUNT 7 // <=== Update this with the number of things this sensor can sense
...
#define SENSOR_I2C_ADDRESSES const uint8_t sensorI2cAddresses[] = {0x18} // <=== Update this with the I2C addresses for this sensor
...
public:
  ...
  bool detectSensor(uint8_t sensorAddress, TwoWire &port)
  {
    ...
    if (device->begin(sensorAddress, port))
      return ((device->readStatus() & 0x5A) == 0x40);
    return (false);
  }
