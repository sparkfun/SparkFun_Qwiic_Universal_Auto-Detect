#ifndef SPARKFUN_QUAD_SENSOR_HEADER_SCD40_H // <=== Update this with the new sensor type
#define SPARKFUN_QUAD_SENSOR_HEADER_SCD40_H // <=== Update this with the new sensor type

#include "SCD4X/SparkFun_SCD4x_Arduino_Library.h" // <=== Update this with the new sensor library header file
...
#define CLASSNAME SCD4x // <=== Update this with the new sensor type

#define CLASSTITLE SFE_QUAD_Sensor_SCD40 // <=== Update this with the new sensor type
...
#define SENSOR_I2C_ADDRESSES const uint8_t sensorI2cAddresses[] = {0x62} // <=== Update this with the I2C addresses for this sensor
...
public:
  ...
  CLASSTITLE(void)
  {
    ...
    _muxAddress = 0;
    _muxPort = 0;
    _classPtr = new CLASSNAME(SCD4x_SENSOR_SCD40);
    _next = NULL;
  ...
  bool detectSensor(uint8_t sensorAddress, TwoWire &port)
  {
    ...
    if (port.endTransmission() == 0)
    {
      ...
      return (device->begin(port, false, _autoCalibrate));
    }
    else
      return (false);
  }
  ...
  {
    CLASSNAME *device = (CLASSNAME *)_classPtr;
    return (device->begin(port, true, _autoCalibrate));
  }
  ...
  const char *getSettingName(uint8_t setting)
  {
    ...
    case 0:
      ...
      break;
    case 1:
      return ("Sensor Altitude (m)");
      break;
    case 2:
      return ("Temperature Offset (C)");
      break;
    default:
      return (NULL);
      break;
