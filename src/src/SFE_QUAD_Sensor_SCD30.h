#ifndef SPARKFUN_QUAD_SENSOR_HEADER_SCD30_H // <=== Update this with the new sensor type
#define SPARKFUN_QUAD_SENSOR_HEADER_SCD30_H // <=== Update this with the new sensor type

#include "SCD30/SparkFun_SCD30_Arduino_Library.h" // <=== Update this with the new sensor library header file
...
#define CLASSNAME SCD30 // <=== Update this with the new sensor type

#define CLASSTITLE SFE_QUAD_Sensor_SCD30 // <=== Update this with the new sensor type
...
#define SENSOR_I2C_ADDRESSES const uint8_t sensorI2cAddresses[] = {0x61} // <=== Update this with the I2C addresses for this sensor
...
public:
  ...
  bool detectSensor(uint8_t sensorAddress, TwoWire &port)
  {
    ...
    return (device->begin(port, true, false));
  }
  ...
  bool getSenseReading(uint8_t sense, char *reading)
  {
    ...
    case 1:
      ...
      break;
    case 2:
      _sprintf._dtostrf((double)device->getHumidity(), reading); // Get the humidity
      return (true);
      break;
    default:
      return (false);
  ...
  bool setSetting(uint8_t setting, SFE_QUAD_Sensor_Every_Type_t *value)
  {
    ...
    {
    case 0:
      device->setMeasurementInterval(value->UINT16_T);
      break;
    case 1:
      device->setAltitudeCompensation(value->UINT16_T);
      break;
    case 2:
  ...
  bool getConfigurationItem(uint8_t configItem, SFE_QUAD_Sensor_Every_Type_t *value)
  {
    ...
    {
    case 0:
      value->UINT16_T = device->getMeasurementInterval();
      break;
    case 1:
      value->UINT16_T = device->getAltitudeCompensation();
      break;
    case 2:
