#ifndef SPARKFUN_QUAD_SENSOR_HEADER_VEML6075_H // <=== Update this with the new sensor type
#define SPARKFUN_QUAD_SENSOR_HEADER_VEML6075_H // <=== Update this with the new sensor type

#include "VEML6075/SparkFun_VEML6075_Arduino_Library.h" // <=== Update this with the new sensor library header file
...
#define CLASSNAME VEML6075 // <=== Update this with the new sensor type

#define CLASSTITLE SFE_QUAD_Sensor_VEML6075 // <=== Update this with the new sensor type
...
public:
  ...
  bool getSenseReading(uint8_t sense, char *reading)
  {
    ...
    {
    case 0:
      _sprintf._dtostrf((double)device->uva(), reading); // Get the pressure
      return (true);
      break;
    case 1:
      _sprintf._dtostrf((double)device->uvb(), reading); // Get the temperature
      return (true);
      break;
    case 2:
      _sprintf._dtostrf((double)device->index(), reading); // Get the humidity
      return (true);
      break;
    default:
      return (false);
