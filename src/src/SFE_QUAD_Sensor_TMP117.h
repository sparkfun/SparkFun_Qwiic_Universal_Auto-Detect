#ifndef SPARKFUN_QUAD_SENSOR_HEADER_TMP117_H // <=== Update this with the new sensor type
#define SPARKFUN_QUAD_SENSOR_HEADER_TMP117_H // <=== Update this with the new sensor type

#include "TMP117/SparkFun_TMP117.h" // <=== Update this with the new sensor library header file
...
#define CLASSNAME TMP117 // <=== Update this with the new sensor type

#define CLASSTITLE SFE_QUAD_Sensor_TMP117 // <=== Update this with the new sensor type
...
#define SENSOR_I2C_ADDRESSES const uint8_t sensorI2cAddresses[] = {0x48, 0x49, 0x4A, 0x4B} // <=== Update this with the I2C addresses for this sensor
...
public:
  ...
  bool initializeSensor(uint8_t sensorAddress, TwoWire &port)
  {
    ...
    {
      CLASSNAME *device = (CLASSNAME *)_classPtr;
      device->setConversionAverageMode(0);
      device->setConversionCycleBit(0);
      device->setContinuousConversionMode();
      return (true);
    }
    else
    {
